...
class LdbSignature():
    ...
    def parse(clazz, sig):
        ...
        for (i, subsig) in enumerate(elements[3:]):
            ...
            '''
                not support Byte Compare Subsignatures
            '''
            ...
            if subsig.count('/') >= 2:
                ...
                '''
                split = subsig.find('/')
                subcondition = subsig[:split]
                condition = condition.replace(str(i),'('+'('+subcondition+')'+'&%d'%i+')')
                subsig = subsig[split:]
                subsignatures.append(SubSignature(parse_offset("*"),PCRE(subsig),""))
                continue
            elems = subsig.split(":")
    ...
    def _parse_target_description_block(clazz, tdb):
        ...
        for element in elements:
            ...
            if k == "Target":
                ...
                # pass
                parsed["container"] = v.split(">")
            elif k == "Intermediates":
                parsed["intermediates"] = v.split(">")
                # pass
            elif k == "HandlerType":
                pass
            elif k == "FileSize":
                # pass
                min, max = v.split("-")
                parsed["filesize"] = (int(min), int(max))
            else:
    ...
    def __init__(self, name, condition, subsignatures, target_type = 0, engine = (1, 255),container=[]):
        self.name = name
